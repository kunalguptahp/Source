...
namespace HP.ElementsCPS.Apps.WebUI.Tests.Fixtures.PageFixtures
{
	...
	public class NoteDetailFixture : SharedIEInstanceTestFixture
	{
		...
		public void AddNewWithOnlyMandatoryTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				string[] componentTypeInfo = { "NewNote","1","EntityTypeOfA"};

				ie.GoTo(pageUri);
				ie.FindCreateButton().Click();
				ie.FindDetailPageTextFieldByControlName("txtName").TypeText(componentTypeInfo[0]);
				ie.FindDetailPageTextFieldByControlName("txtEntityId").TypeText(componentTypeInfo[1]);
				ie.FindDetailPageSelectListByControlName("ddlEntityType").Select(componentTypeInfo[2]);
		...
		public void AddNewWithAllFieldsTest()
		{
			...
			{
				Uri pageUri = ElementsCPSWatinUtility.CreateElementsCPSUri("DataAdmin/NoteList.aspx");
				string[] componentTypeInfo = { "NewNote", "New", "Inactive", "1", "EntityTypeOfA" };
				...
				ie.FindDetailPageTextFieldByControlName("txtEntityId").TypeText(componentTypeInfo[3]);
				ie.FindDetailPageSelectListByControlName("ddlEntityType").Select(componentTypeInfo[4]);
				ie.FindSaveButton().Click();
		...
		public void EditTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				Assert.AreEqual(ie.Span(Find.ById(Constants.ClientIdDetailPanelPrefix + "lblIdValue")).Text, "1");
			}
		}
		...
		public void RequiredFieldValidatorTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				ElementsCPSWatinUtility.AssertPageBodyTextContains(@"Please select a Entity Type.", ie);
				ElementsCPSWatinUtility.AssertPageBodyTextContains(@"Please enter an Entity Id.", ie);
