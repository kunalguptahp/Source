...
namespace HP.ElementsCPS.Apps.WebUI.Tests.Fixtures.PageFixtures
{
	...
	public class RedirectorTypeDetailFixture : SharedIEInstanceTestFixture
	{
		...
		public void AddNewWithOnlyMandatoryTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				string[] RedirectorTypeInfo = { "NewRedirectorType","DomainOfA","GroupTypeOfA","10"};
				...
				ie.FindListPageTextFieldByControlName("txtName").TypeText(RedirectorTypeInfo[0]);
				ie.FindFilterButton().Click();
				ie.FindButtonByText(Constants.ButtonText_EditDotDotDot).Click();

				ElementsCPSWatinUtility.VerifyCreateSuccess(ie,RedirectorTypeInfo, ElementsCPSWatinUtility.Creator, DateTime.Now.ToUniversalTime().ToUniversalTime(), ElementsCPSWatinUtility.Modifier, DateTime.Now.ToUniversalTime().ToUniversalTime());
		...
		public void AddNewWithAllFieldsTest()
		{
			...
			{
				Uri pageUri = ElementsCPSWatinUtility.CreateElementsCPSUri("DataAdmin/ProxyURLTypeList.aspx");
				string[] RedirectorTypeInfo = { "NewRedirectorType", "New", "Inactive", "DomainOfA", "GroupTypeOfA","10" };

				ie.GoTo(pageUri);
				ie.FindCreateButton().Click();
				ie.FindDetailPageTextFieldByControlName("txtName").TypeText(RedirectorTypeInfo[0]);
				ie.FindDetailPageTextFieldByControlName("txtDescription").TypeText(RedirectorTypeInfo[1]);
				ie.FindDetailPageSelectListByControlName("ddlStatus").Select(RedirectorTypeInfo[2]);
				ie.FindDetailPageSelectListByControlName("ddlDomain").Select(RedirectorTypeInfo[3]);
				ie.FindDetailPageSelectListByControlName("ddlGroupType").Select(RedirectorTypeInfo[4]);

				ie.FindDetailPageTextFieldByControlName("txtElementsKey").TypeText(RedirectorTypeInfo[5]); 
				ie.FindSaveButton().Click();
				ie.FindSaveButton().Click();
		...
		public void EditTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				ie.FindFilterButton().Click();
				ie.FindButtonByText(Constants.ButtonText_EditDotDotDot).Click();
				ElementsCPSWatinUtility.VerifyModifySuccess(ie, RedirectorTypeInfo, ElementsCPSWatinUtility.Modifier, DateTime.Now.ToUniversalTime().ToUniversalTime());
		...
		public void CancelButtonTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				ElementsCPSWatinUtility.AssertPageBodyTextExcludes(RedirectorTypeInfo[0], ie);
			}
		}
