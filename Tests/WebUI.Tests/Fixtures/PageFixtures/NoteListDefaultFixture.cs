...
namespace HP.ElementsCPS.Apps.WebUI.Tests.Fixtures.PageFixtures
{
	...
	public class NoteListDefaultFixture : SharedIEInstanceTestFixture
	{

		string[] expectedStringsPrefixes = { "NoteOfA", "NoteOfB", "NoteOfC", "NoteOfD", "NoteOfE" };
		string[] expectedStringsSuffixes = { "Active.NoteStatus", "Inactive.NoteStatus", "Deleted.NoteStatus" };
		...
		#region Tests: Happy Path
			...
			[Test]
		public void EntityTypeFilterListTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				// Active status test
				ie.FindListPageSelectListByControlName(("ddlEntityType")).Select("EntityTypeOfA");
				...
				ie.DoClickFilterAreaExpansionToggle();
				// Active status test
				ie.FindListPageSelectListByControlName(("ddlEntityType")).Select("- All -");
				ie.FindFilterButton().Click();
				ElementsCPSWatinUtility.AssertPageBodyHtmlContains(expectedStringsPrefixes, ie);
				ie.DoClickFilterAreaExpansionToggle();
				// Inactive status test
				ie.FindListPageSelectListByControlName(("ddlEntityType")).Select("EntityTypeOfB");
				ie.FindFilterButton().Click();
		...
		public void StatusFilterListTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				ie.DoClickFilterAreaExpansionToggle();
				// filter by emails starting with "person."
				ie.FindListPageTextFieldByControlName(("txtName")).TypeText("%NoteStatus");
		...
		public void NameFilterListTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				ie.FindFilterButton().Click();
				ElementsCPSWatinUtility.AssertPageBodyHtmlContains("There are no Notes found", ie);
			}
		}

		[Test]
		public void EntityIdFilterListTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				Uri pageUri = ElementsCPSWatinUtility.CreateElementsCPSUri("DataAdmin/NoteList.aspx");
				...
				// single name test
				ie.FindListPageTextFieldByControlName(("txtName")).TypeText("NoteOf");
				ie.FindListPageTextFieldByControlName(("txtEntityId")).TypeText("1");
				ie.FindFilterButton().Click();
				ElementsCPSWatinUtility.AssertPageBodyHtmlContains(expectedStringsPrefixes[0], ie);
				ElementsCPSWatinUtility.AssertPageBodyHtmlExcludes(expectedStringsPrefixes[1], ie);
				ElementsCPSWatinUtility.AssertPageBodyHtmlExcludes(expectedStringsPrefixes[2], ie);
				ie.DoClickFilterAreaExpansionToggle();
				// name "starting with" test
				ie.FindListPageTextFieldByControlName(("txtEntityId")).TypeText("2");
				ie.FindFilterButton().Click();
				ElementsCPSWatinUtility.AssertPageBodyHtmlContains(expectedStringsPrefixes[1], ie);
				ElementsCPSWatinUtility.AssertPageBodyHtmlExcludes(expectedStringsPrefixes[0], ie);
				ElementsCPSWatinUtility.AssertPageBodyHtmlExcludes(expectedStringsPrefixes[2], ie);
		...
		public void AllFieldsFilterListTest()
		{
			EnhancedIE ie = this.SharedIEInstance;
			{
				...
				ie.FindListPageTextFieldByControlName(("txtEntityId")).TypeText("1");
				ie.FindListPageTextFieldByControlName(("txtIdList")).TypeText("1");
				ie.FindListPageTextFieldByControlName(("txtName")).TypeText("NoteOfA");
				ie.FindListPageSelectListByControlName(("ddlStatus")).Select("Active");
