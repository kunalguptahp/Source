...
{
	public partial class WorkflowModuleCategoryUpdatePanel : RecordDetailUserControl
	{
		#region Properties
...
//				this.UpdateChildListImmutableConditions();
		...
		{
			WorkflowModuleCategoryQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					WorkflowModuleCategory bindItem = WorkflowModuleCategory.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
		...
		{
			WorkflowModuleCategory saveItem;
			if (this.DataSourceId != null)
			{
				saveItem = WorkflowModuleCategory.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new WorkflowModuleCategory(true);
			}
            ...
            Global.RemoveWorkflowModuleCategoryListControl(RowStatus.RowStatusId.Active);
		}
