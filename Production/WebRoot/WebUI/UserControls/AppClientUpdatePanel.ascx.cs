...
{
    public partial class AppClientUpdatePanel : RecordDetailUserControl
    {
        #region Properties
        ...
        protected void cvValidateExistName_ServerValidate(object source, ServerValidateEventArgs args)
        {
            if (!this.IsNewRecord)
            {
                AppClient bindItem = AppClient.FetchByID(this.DataSourceId);
                args.IsValid = AppClient.AreValidNames(bindItem.Id, args.Value);
            }
            else
            {
                args.IsValid = AppClient.AreValidNames(args.Value);
            }
            //args.IsValid = AppClient.AreValidNames(args.Value);
        }
        #endregion
        ...
        {
            AppClientQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
        ...
        protected override void SaveInput()
        {
            ...
            {
                saveItem = AppClient.FetchByID(this.DataSourceId);
            }
            else
            {
                saveItem = new AppClient(true);
            }
            ...
            Global.RemoveAppClientListControl(RowStatus.RowStatusId.Active);
        }
