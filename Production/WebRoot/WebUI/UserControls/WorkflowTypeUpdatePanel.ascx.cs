...
{
	public partial class WorkflowTypeUpdatePanel : RecordDetailUserControl
	{
		#region Properties
		...
		{
			WorkflowTypeQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					WorkflowType bindItem = WorkflowType.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
		...
		{
			WorkflowType saveItem;
			if (this.DataSourceId != null)
			{
				saveItem = WorkflowType.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new WorkflowType(true);
			}
            ...
            Global.RemoveWorkflowTypeListControl(RowStatus.RowStatusId.Active);
        }
