...
{
	public partial class WorkflowTypeListPanel : BaseListViewUserControl
	{
		...
		{
			WorkflowTypeQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			WorkflowTypeQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			WorkflowTypeQuerySpecification query = new WorkflowTypeQuerySpecification();
			GridView gv = this.Grid;
		...
		{
			this.EditWorkflowType(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapWorkflowType, VwMapWorkflowTypeCollection, WorkflowTypeQuerySpecification>(querySpecification, VwMapWorkflowTypeController.Fetch));
		}
		...
		/// </summary>
		public WorkflowTypeQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		private void EditWorkflowType(int? workflowTypeId)
		{
			this.Response.Redirect(Global.GetWorkflowTypeUpdatePageUri(workflowTypeId, this.QuerySpecification));
		}
		...
		/// <returns></returns>
		public WorkflowTypeQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as WorkflowTypeQuerySpecification ?? new WorkflowTypeQuerySpecification(original);
		}
