...
{
	public partial class ConfigurationServiceLabelListPanel : BaseListViewUserControl
	{
		#region Overrides
		...
		{
			ConfigurationServiceLabelQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			ConfigurationServiceLabelQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
			...
			this.ddlItem.DisableIf(immutableConditions.ConfigurationServiceItemId != null);
			if (query.ConfigurationServiceItemId == null)
			{
				this.ddlItem.ClearSelection();
			}
			else
			{
				this.ddlItem.ForceSelectedValue(query.ConfigurationServiceItemId);
			}
		...
		{
			ConfigurationServiceLabelQuerySpecification query = new ConfigurationServiceLabelQuerySpecification();
			GridView gv = this.Grid;
			...
			query.ConfigurationServiceItemId = this.ddlItem.SelectedValue.TryParseInt32();
		...
		{
			this.EditLabel(this.GetRowIdInt32(index));
		}
		...
		protected override void PopulateListControls()
		{
			...
			Global.BindConfigurationServiceItemListControl(this.ddlItem, null);
			this.ddlItem.InsertItem(0, "", Global.GetAllListText());
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapConfigurationServiceLabel, VwMapConfigurationServiceLabelCollection, ConfigurationServiceLabelQuerySpecification>(querySpecification, VwMapConfigurationServiceLabelController.Fetch));
		}
		...
		/// </summary>
		public ConfigurationServiceLabelQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		private void EditLabel(int? labelId)
		{
			this.Response.Redirect(Global.GetConfigurationServiceLabelUpdatePageUri(labelId, this.QuerySpecification));
		}
