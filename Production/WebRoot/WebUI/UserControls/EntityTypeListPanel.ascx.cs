...
{
	public partial class EntityTypeListPanel : BaseListViewUserControl
	{
		...
		protected override void BindScreen(IQuerySpecification querySpecification)
		{
			...
			EntityTypeQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			EntityTypeQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			EntityTypeQuerySpecification query = new EntityTypeQuerySpecification();
			GridView gv = this.Grid;
		...
		{
			this.EditEntityType(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapEntityType, VwMapEntityTypeCollection, EntityTypeQuerySpecification>(querySpecification, VwMapEntityTypeController.Fetch));
		}
		...
		/// </summary>
		public EntityTypeQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		private void EditEntityType(int? recordId)
		{
			this.Response.Redirect(Global.GetEntityTypeUpdatePageUri(recordId, this.QuerySpecification));
		}
