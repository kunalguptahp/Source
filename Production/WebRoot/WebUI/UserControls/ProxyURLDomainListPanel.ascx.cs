...
{
	public partial class ProxyURLDomainListPanel : BaseListViewUserControl
	{
		...
		{
			ProxyURLDomainQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			ProxyURLDomainQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			ProxyURLDomainQuerySpecification query = new ProxyURLDomainQuerySpecification();
			GridView gv = this.Grid;
		...
		{
			this.EditDomain(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapProxyURLDomain, VwMapProxyURLDomainCollection, ProxyURLDomainQuerySpecification>(querySpecification, VwMapProxyURLDomainController.Fetch));
		}
		...
		/// </summary>
		public ProxyURLDomainQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			this.Response.Redirect(Global.GetProxyURLDomainUpdatePageUri(domainId, this.QuerySpecification));
		}
		...
		/// <returns></returns>
		public ProxyURLDomainQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as ProxyURLDomainQuerySpecification ?? new ProxyURLDomainQuerySpecification(original);
		}
