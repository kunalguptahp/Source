...
{
	public partial class QueryParameterListPanel : BaseListViewUserControl
	{
		...
		{
			QueryParameterQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			QueryParameterQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			QueryParameterQuerySpecification query = new QueryParameterQuerySpecification();
			GridView gv = this.Grid;
		...
		{
			this.EditQueryParameter(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapQueryParameter, VwMapQueryParameterCollection, QueryParameterQuerySpecification>(querySpecification, VwMapQueryParameterController.Fetch));
		}
		...
		/// </summary>
		public QueryParameterQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			// don't allow dataAdmin to create a new query parameter
		...
		private void EditQueryParameter(int? recordId)
		{
			this.Response.Redirect(Global.GetQueryParameterUpdatePageUri(recordId, this.QuerySpecification));
		}
		...
		/// <returns></returns>
		public QueryParameterQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as QueryParameterQuerySpecification ?? new QueryParameterQuerySpecification(original);
		}
