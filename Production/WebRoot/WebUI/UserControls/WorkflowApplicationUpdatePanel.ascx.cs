...
{
	public partial class WorkflowApplicationUpdatePanel : RecordDetailUserControl
	{
...
//				this.UpdateChildListImmutableConditions();
		...
		{
			Global.BindWorkflowApplicationTypeListControl(this.ddlApplicationType, RowStatus.RowStatusId.Active);
		...
		{
			WorkflowApplicationQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					WorkflowApplication bindItem = WorkflowApplication.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
					...
					this.ddlApplicationType.ForceSelectedValue(bindItem.WorkflowApplicationTypeId);
		...
		{
			WorkflowApplication saveItem;
			if (this.DataSourceId != null)
			{
				saveItem = WorkflowApplication.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new WorkflowApplication(true);
			}
			...
			saveItem.WorkflowApplicationTypeId = Convert.ToInt32(this.ddlApplicationType.SelectedValue);
            ...
            Global.RemoveWorkflowApplicationListControl(RowStatus.RowStatusId.Active);
        }
