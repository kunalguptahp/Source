...
{
	public partial class ConfigurationServiceGroupCopyPanel : BaseQuerySpecificationEditDataUserControl
	{
		#region Constants
		...
		protected override void SaveInput()
		{
			...
			ConfigurationServiceGroupCollection originalConfigurationServiceGroups = this.GetConfigurationServiceGroups();

			foreach (ConfigurationServiceGroup originalItem in originalConfigurationServiceGroups)
			{
				ConfigurationServiceGroup newItem = originalItem.SaveAsNew(true, true);
				...
				newItem.ConfigurationServiceGroupStatusId = (int)ConfigurationServiceGroupStateId.Modified;
				newItem.ValidationId = null;
				newItem.ProductionId = null;
				newItem.Save(Core.Security.SecurityManager.CurrentUserIdentity.Name);

				cpsLogList.Add(new CPSLog(newItem.Id, string.Format(CultureInfo.CurrentCulture, "Copied Id #{0} to #{1}", originalItem.Id, newItem.Id), "New Configuration Service Group Name: " + newItem.Name, "Copy success"));
