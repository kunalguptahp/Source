...
{
	public partial class JumpstationGroupSelectorListPanel : BaseListViewUserControl
	{

        #region Constants

        private const string NOT_SELECT_MESSAGE = "There is no row selected.";
		...
		{
			JumpstationGroupSelectorQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			JumpstationGroupSelectorQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
			...
			this.hdnJumpstationGroupId.Value = immutableConditions.JumpstationGroupId.ToString();
		...
		{
			JumpstationGroupSelectorQuerySpecification query = new JumpstationGroupSelectorQuerySpecification();
			GridView gv = this.Grid;
			...
			query.JumpstationGroupId = this.hdnJumpstationGroupId.Value.TryParseInt32();
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapJumpstationGroupSelector, VwMapJumpstationGroupSelectorCollection, JumpstationGroupSelectorQuerySpecification>(querySpecification, VwMapJumpstationGroupSelectorController.Fetch));
		}
		...
		/// </summary>
		public JumpstationGroupSelectorQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			List<int> selectedJumpstationGroupSelectorIds = this.GetSelectedJumpstationGroupSelectorIds();
			if (selectedJumpstationGroupSelectorIds.Count == 0)
			{
				WebUtility.ShowAlertBox(this, NOT_SELECT_MESSAGE);
			}
			else
			{
				JumpstationGroupSelectorQuerySpecification jumpstationGroupSelectorQuerySpecification = new JumpstationGroupSelectorQuerySpecification() { IdList = selectedJumpstationGroupSelectorIds };
				this.CopyJumpstationGroupSelectors(jumpstationGroupSelectorQuerySpecification);
			}
		}
		...
		{
			this.Response.Redirect(Global.GetJumpstationGroupSelectorUpdatePageUri(groupSelectorId, this.QuerySpecification));
		}

		private void CopyJumpstationGroupSelectors(JumpstationGroupSelectorQuerySpecification jumpstationGroupSelectorQuerySpecification)
		{
			this.Response.Redirect(Global.GetJumpstationGroupSelectorCopyPageUri(jumpstationGroupSelectorQuerySpecification));
		}

		private List<int> GetSelectedJumpstationGroupSelectorIds()
		{
