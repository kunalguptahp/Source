...
{
	public partial class WorkflowApplicationListPanel : BaseListViewUserControl
	{
		...
		{
			WorkflowApplicationQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			WorkflowApplicationQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			WorkflowApplicationQuerySpecification query = new WorkflowApplicationQuerySpecification();
			GridView gv = this.Grid;
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(query, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapWorkflowApplication, VwMapWorkflowApplicationCollection, WorkflowApplicationQuerySpecification>(query, VwMapWorkflowApplicationController.Fetch));
		}
		...
		/// </summary>
		public WorkflowApplicationQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			this.Response.Redirect(Global.GetWorkflowApplicationUpdatePageUri(applicationId, this.QuerySpecification));
		}
		...
		/// <returns></returns>
		public WorkflowApplicationQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as WorkflowApplicationQuerySpecification ?? new WorkflowApplicationQuerySpecification(original);
		}
