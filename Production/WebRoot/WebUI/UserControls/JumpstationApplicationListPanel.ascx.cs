...
{
	public partial class JumpstationApplicationListPanel : BaseListViewUserControl
	{
		...
		{
			JumpstationApplicationQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			JumpstationApplicationQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			JumpstationApplicationQuerySpecification query = new JumpstationApplicationQuerySpecification();
			GridView gv = this.Grid;
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(query, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapJumpstationApplication, VwMapJumpstationApplicationCollection, JumpstationApplicationQuerySpecification>(query, VwMapJumpstationApplicationController.Fetch));
		}
		...
		/// </summary>
		public JumpstationApplicationQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			this.Response.Redirect(Global.GetJumpstationApplicationUpdatePageUri(groupTypeId, this.QuerySpecification));
		}
		...
		/// <returns></returns>
		public JumpstationApplicationQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as JumpstationApplicationQuerySpecification ?? new JumpstationApplicationQuerySpecification(original);
		}
