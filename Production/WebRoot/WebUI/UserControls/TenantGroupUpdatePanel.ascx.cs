...
{
    public partial class TenantGroupUpdatePanel : RecordDetailUserControl
    {
        #region Properties
        ...
        protected void cvValidateExistName_ServerValidate(object source, ServerValidateEventArgs args)
        {
            if (!this.IsNewRecord)
            {
                Tenant bindItem = Tenant.FetchByID(this.DataSourceId);
                args.IsValid = Tenant.AreValidNames(bindItem.Id,args.Value); 
            }
            else
            {
                args.IsValid = Tenant.AreValidNames(args.Value);
            }
        ...
        private void PopulateListControls()
        {
            ...
            {
                tenantId = this.DataSourceId.Value;
            }
            Global.BindAppClientListControl(this.chkClientList, RowStatus.RowStatusId.Active, tenantId);
        ...
        private void ClearDataControls()
        {
            ...
            this.chkClientList.ClearSelection();
        ...
        {
            TenantQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
        ...
        /// <returns></returns>
        public TenantQuerySpecification ConvertToExpectedType(IQuerySpecification original)
        {
            if (original == null)
            {
                return null;
            ...
            return original as TenantQuerySpecification ?? new TenantQuerySpecification(original);
        }
        ...
        protected override void BindItem()
        {
            ...
            else
            {
                ...
                {
                    Tenant bindItem = Tenant.FetchByID(this.DataSourceId);
                    //TenantGroup bindItem = TenantGroupController.FetchByID(Int32.Parse(lblIdValue.Text))[0];
                    if (bindItem == null)
                    {
                        this.Visible = false;
                        return;
                    ...
                    this.chkClientList.ClearSelection();
                    Global.SetAppClientListControl(this.chkClientList, bindItem.GetAppClientCollection());
        ...
        protected override void SaveInput()
        {
            ...
            if (this.DataSourceId != null)
            {
                saveItem = Tenant.FetchByID(this.DataSourceId);
            }
            else
            {
                saveItem = new Tenant(true);
            }
            ...
            //create a list of the selected AppClient
            List<AppClientId> newAppClients = new List<AppClientId>();
            foreach (ListItem item in this.chkClientList.Items)
            {
                if (item.Selected)
                {
                    int AppClientId = Convert.ToInt32(item.Value);
                    newAppClients.Add((AppClientId)AppClientId);
                }
            }
          
            saveItem.SetAppClients(newAppClients);
            ...
            //Global.RemoveTenantGroupListControl(RowStatus.RowStatusId.Active);
        }
