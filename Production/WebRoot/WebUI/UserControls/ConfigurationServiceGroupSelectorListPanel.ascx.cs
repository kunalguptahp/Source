...
{
	public partial class ConfigurationServiceGroupSelectorListPanel : BaseListViewUserControl
	{
		...
		{
			ConfigurationServiceGroupSelectorQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			ConfigurationServiceGroupSelectorQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
			...
			this.hdnConfigurationServiceGroupId.Value = immutableConditions.ConfigurationServiceGroupId.ToString();
			...
			this.btnCreate.Enabled = isDataModificationAllowed;
			this.btnCopy.Enabled = isDataModificationAllowed;
		}
		...
		{
			ConfigurationServiceGroupSelectorQuerySpecification query = new ConfigurationServiceGroupSelectorQuerySpecification();
			GridView gv = this.Grid;
			...
			query.ConfigurationServiceGroupId = this.hdnConfigurationServiceGroupId.Value.TryParseInt32();
		...
		{
			this.EditGroupSelector(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapConfigurationServiceGroupSelector, VwMapConfigurationServiceGroupSelectorCollection, ConfigurationServiceGroupSelectorQuerySpecification>(querySpecification, VwMapConfigurationServiceGroupSelectorController.Fetch));
		}
		...
		/// </summary>
		public ConfigurationServiceGroupSelectorQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			List<int> selectedConfigurationServiceGroupSelectorIds = this.GetSelectedConfigurationServiceGroupSelectorIds();
			if (selectedConfigurationServiceGroupSelectorIds.Count == 0)
			{
				WebUtility.ShowAlertBox(this, NOT_SELECT_MESSAGE);
			}
			else
			{
				ConfigurationServiceGroupSelectorQuerySpecification configurationServiceGroupSelectorQuerySpecification = new ConfigurationServiceGroupSelectorQuerySpecification() { IdList = selectedConfigurationServiceGroupSelectorIds };
				this.CopyConfigurationServiceGroupSelectors(configurationServiceGroupSelectorQuerySpecification);
			}
		}
		...
		private void EditGroupSelector(int? groupSelectorId)
		{
			this.Response.Redirect(Global.GetConfigurationServiceGroupSelectorUpdatePageUri(groupSelectorId, this.QuerySpecification));
		}

		private void CopyConfigurationServiceGroupSelectors(ConfigurationServiceGroupSelectorQuerySpecification configurationServiceGroupSelectorQuerySpecification)
		{
			this.Response.Redirect(Global.GetConfigurationServiceGroupSelectorCopyPageUri(configurationServiceGroupSelectorQuerySpecification));
		}

		private List<int> GetSelectedConfigurationServiceGroupSelectorIds()
		{
