...
{
	public partial class QueryParameterUpdatePanel : RecordDetailUserControl
	{
		...
		private void ClearDataControls()
		{
			...
			this.txtElementKey.Text = string.Empty;
		...
		{
			QueryParameterQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					QueryParameter bindItem = QueryParameter.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
					...
					this.txtElementKey.Text = bindItem.ElementsKey;
		...
		{
			QueryParameter saveItem;
			if (this.DataSourceId != null)
			{
				saveItem = QueryParameter.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new QueryParameter(true);
			}
			...
			saveItem.ElementsKey = this.txtElementKey.Text.TrimToNull();
		...
		{
			this.UpdateQueryParameterValueListImmutableConditions();
		}

		private void UpdateQueryParameterValueListImmutableConditions()
		{
			this.pnlQueryParameterValueListPanel.Visible = this.DataSourceId != null;
			this.ucQueryParameterValueList.ImmutableQueryConditions = new QueryParameterValueQuerySpecification {QueryParameterId = (this.DataSourceId ?? -1), Wildcard = true};
		}
