...
{
	public partial class ProxyURLTypeListPanel : BaseListViewUserControl
	{
		...
		{
			ProxyURLTypeQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			ProxyURLTypeQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			ProxyURLTypeQuerySpecification query = new ProxyURLTypeQuerySpecification();
			GridView gv = this.Grid;
		...
		{
			this.EditProxyURLType(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapProxyURLType, VwMapProxyURLTypeCollection, ProxyURLTypeQuerySpecification>(querySpecification, VwMapProxyURLTypeController.Fetch));
		}
		...
		/// </summary>
		public ProxyURLTypeQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			// don't allow dataAdmin to create a new proxyURL type
		...
		private void EditProxyURLType(int? ProxyURLTypeId)
		{
			this.Response.Redirect(Global.GetProxyURLTypeUpdatePageUri(ProxyURLTypeId, this.QuerySpecification));
		}
