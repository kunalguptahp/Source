...
{
	public partial class WorkflowQueryParameterValueEditListUpdatePanel : BaseListViewUserControl
	{
		...
		{
			int WorkflowSelectorId = this.GetWorkflowSelectorIdOfParent() ?? 0;
			if (WorkflowSelectorId == 0)
			{
				SelectionList = null;
				return;
			}
			...
			VwMapWorkflowSelectorQueryParameterValueCollection qpvCollection =
				VwMapWorkflowSelectorQueryParameterValueController.FetchByWorkflowSelectorIdQueryParameterId(WorkflowSelectorId, queryParameterId);
			foreach (VwMapWorkflowSelectorQueryParameterValue queryParameterValue in qpvCollection)
			{
		...
		protected void btnSave_Click(object sender, EventArgs e)
		{
			...
			int? WorkflowSelectorId = GetWorkflowSelectorIdOfParent();
			if (WorkflowSelectorId != null)
			{
				this.SaveInput((int)WorkflowSelectorId, queryParameterId);			
			}
		}
		...
		/// </summary>
		/// <returns></returns>
		public void SaveInput(int WorkflowSelectorId, int queryParameterId)
		{
			try
			{
				...
				using (TransactionScope scope = new TransactionScope())
				{
					...
					WorkflowSelectorQueryParameterValue.DestroyByWorkflowSelectorIdQueryParameterId(WorkflowSelectorId, queryParameterId);

					// update selected query parameter values
					List<int> selectionList = (SelectionList ?? new List<int>());
					for (int i = 0; i < selectionList.Count; i++)
					{
						WorkflowSelectorQueryParameterValue saveItem = new WorkflowSelectorQueryParameterValue(true);
						saveItem.WorkflowSelectorId = WorkflowSelectorId;
		...
		private int? GetWorkflowSelectorIdOfParent()
		{
