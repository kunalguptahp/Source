...
{
	public partial class WorkflowConditionListPanel : BaseListViewUserControl
	{
		#region Overrides
		...
		{
			this.EditCondition(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapWorkflowCondition, VwMapWorkflowConditionCollection, WorkflowConditionQuerySpecification>(querySpecification, VwMapWorkflowConditionController.Fetch));
		}
		...
		/// </summary>
		public WorkflowConditionQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			this.Response.Redirect(Global.GetWorkflowConditionUpdatePageUri(ConditionId, this.QuerySpecification));
		}
