...
{
	public partial class NoteTypeListPanel : BaseListViewUserControl
	{
		...
		protected override void BindScreen(IQuerySpecification querySpecification)
		{
			...
			NoteTypeQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			NoteTypeQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			NoteTypeQuerySpecification query = new NoteTypeQuerySpecification();
			GridView gv = this.Grid;
		...
		{
			this.EditNoteType(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapNoteType, VwMapNoteTypeCollection, NoteTypeQuerySpecification>(querySpecification, VwMapNoteTypeController.Fetch));
		}
		...
		/// </summary>
		public NoteTypeQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			const string entityName = "NoteTypes";
		...
		private void EditNoteType(int? recordId)
		{
			this.Response.Redirect(Global.GetNoteTypeUpdatePageUri(recordId, this.QuerySpecification));
		}
		...
		/// <returns></returns>
		public NoteTypeQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as NoteTypeQuerySpecification ?? new NoteTypeQuerySpecification(original);
		}
