...
{
	public partial class ProxyURLDomainUpdatePanel : RecordDetailUserControl
	{
		#region Properties
...
//				this.UpdateChildListImmutableConditions();
		...
		private void ClearDataControls()
		{
			...
			this.txtValidationDomain.Text = string.Empty;
			this.txtPublicationDomain.Text = string.Empty;
		}
		/// <summary>
		...
		{
			ProxyURLDomainQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					ProxyURLDomain bindItem = ProxyURLDomain.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
					...
					this.txtValidationDomain.Text = bindItem.ValidationDomain;
					this.txtPublicationDomain.Text = bindItem.ProductionDomain;
		...
		{
			ProxyURLDomain saveItem;
			if (this.DataSourceId != null)
			{
				saveItem = ProxyURLDomain.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new ProxyURLDomain(true);
			}
			...
			saveItem.ValidationDomain = this.txtValidationDomain.Text.TrimToNull();
            saveItem.ProductionDomain = this.txtPublicationDomain.Text.TrimToNull();
            ...
            Global.RemoveProxyURLDomainListControl(RowStatus.RowStatusId.Active);
        }
