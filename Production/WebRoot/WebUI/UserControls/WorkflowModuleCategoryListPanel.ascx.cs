...
{
	public partial class WorkflowModuleCategoryListPanel : BaseListViewUserControl
	{
		...
		{
			WorkflowModuleCategoryQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			WorkflowModuleCategoryQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			WorkflowModuleCategoryQuerySpecification query = new WorkflowModuleCategoryQuerySpecification();
			GridView gv = this.Grid;
		...
		{
			this.EditModuleCategory(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapWorkflowModuleCategory, VwMapWorkflowModuleCategoryCollection, WorkflowModuleCategoryQuerySpecification>(querySpecification, VwMapWorkflowModuleCategoryController.Fetch));
		}
		...
		/// </summary>
		public WorkflowModuleCategoryQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		private void EditModuleCategory(int? ModuleCategoryId)
		{
			this.Response.Redirect(Global.GetWorkflowModuleCategoryUpdatePageUri(ModuleCategoryId, this.QuerySpecification));
		}
		...
		/// <returns></returns>
		public WorkflowModuleCategoryQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as WorkflowModuleCategoryQuerySpecification ?? new WorkflowModuleCategoryQuerySpecification(original);
		}
