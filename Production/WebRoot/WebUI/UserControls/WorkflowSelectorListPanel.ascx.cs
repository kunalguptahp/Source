...
{
	public partial class WorkflowSelectorListPanel : BaseListViewUserControl
	{
		...
		{
			WorkflowSelectorQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			WorkflowSelectorQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
			...
			this.hdnWorkflowId.Value = immutableConditions.WorkflowId.ToString();
		...
		protected bool IsDataModificationAllowed()
		{
			...
			Workflow csgItem = Workflow.FetchByID(this.hdnWorkflowId.Value.TryParseInt32() ?? 0);
			if (csgItem == null)
				return false;
		...
		{
			WorkflowSelectorQuerySpecification query = new WorkflowSelectorQuerySpecification();
			GridView gv = this.Grid;
			...
			query.WorkflowId = this.hdnWorkflowId.Value.TryParseInt32();
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapWorkflowSelector, VwMapWorkflowSelectorCollection, WorkflowSelectorQuerySpecification>(querySpecification, VwMapWorkflowSelectorController.Fetch));
		}
		...
		/// </summary>
		public WorkflowSelectorQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			List<int> selectedWorkflowSelectorIds = this.GetSelectedWorkflowSelectorIds();
			if (selectedWorkflowSelectorIds.Count == 0)
			{
				WebUtility.ShowAlertBox(this, NOT_SELECT_MESSAGE);
			}
			else
			{
				WorkflowSelectorQuerySpecification configurationServiceGroupSelectorQuerySpecification = new WorkflowSelectorQuerySpecification() { IdList = selectedWorkflowSelectorIds };
				this.CopyWorkflowSelectors(configurationServiceGroupSelectorQuerySpecification);
			}
		}
		...
		{
			this.Response.Redirect(Global.GetWorkflowSelectorUpdatePageUri(groupSelectorId, this.QuerySpecification));
		}

		private void CopyWorkflowSelectors(WorkflowSelectorQuerySpecification configurationServiceGroupSelectorQuerySpecification)
		{
			this.Response.Redirect(Global.GetWorkflowSelectorCopyPageUri(configurationServiceGroupSelectorQuerySpecification));
		}

		private List<int> GetSelectedWorkflowSelectorIds()
		{
