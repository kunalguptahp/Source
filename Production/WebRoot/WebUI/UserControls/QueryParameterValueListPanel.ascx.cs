...
{
	public partial class QueryParameterValueListPanel : BaseListViewUserControl
	{
		...
		protected override void BindScreen(IQuerySpecification querySpecification)
		{
			...
			this.ddlParameter.DisableIf(immutableConditions.QueryParameterId != null);
			if (query.QueryParameterId == null)
			{
				this.ddlParameter.ClearSelection();
			}
			else
			{
				this.ddlParameter.ForceSelectedValue(query.QueryParameterId);
			}
		...
		protected override IQuerySpecification CreateQueryFromScreen()
		{
			...
			query.QueryParameterId = this.ddlParameter.SelectedValue.TryParseInt32();
		...
		{
			this.EditQueryParameterValue(this.GetRowIdInt32(index));
		}
		...
		protected override void PopulateListControls()
		{
			...
			this.ddlParameter.InsertItem(0, "", Global.GetAllListText());
		...
		/// </summary>
		public QueryParameterValueQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			this.EditQueryParameterValue(null);
		}
		...
		private void EditQueryParameterValue(int? recordId)
		{
			this.Response.Redirect(Global.GetQueryParameterValueUpdatePageUri(recordId, this.QuerySpecification));
		}
