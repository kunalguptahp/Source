...
{
	public partial class JumpstationQueryParameterValueEditListUpdatePanel : BaseListViewUserControl
	{
		...
		{
			int jumpstationGroupSelectorId = this.GetJumpstationGroupSelectorIdOfParent() ?? 0;
			if (jumpstationGroupSelectorId == 0)
			{
				SelectionList = null;
				return;
			}
			...
			VwMapJumpstationGroupSelectorQueryParameterValueCollection qpvCollection =
				VwMapJumpstationGroupSelectorQueryParameterValueController.FetchByJumpstationGroupSelectorIdQueryParameterId(jumpstationGroupSelectorId, queryParameterId);
			foreach (VwMapJumpstationGroupSelectorQueryParameterValue queryParameterValue in qpvCollection)
			{
		...
		protected void btnSave_Click(object sender, EventArgs e)
		{
			...
			int? jumpstationGroupSelectorId = GetJumpstationGroupSelectorIdOfParent();
			if (jumpstationGroupSelectorId != null)
			{
				this.SaveInput((int)jumpstationGroupSelectorId, queryParameterId);			
			}
		}
		...
		/// </summary>
		/// <returns></returns>
		public void SaveInput(int jumpstationGroupSelectorId, int queryParameterId)
		{
			try
			{
				...
				using (TransactionScope scope = new TransactionScope())
				{
					...
					JumpstationGroupSelectorQueryParameterValue.DestroyByJumpstationGroupSelectorIdQueryParameterId(jumpstationGroupSelectorId, queryParameterId);

					// update selected query parameter values
					List<int> selectionList = (SelectionList ?? new List<int>());
					for (int i = 0; i < selectionList.Count; i++)
					{
						JumpstationGroupSelectorQueryParameterValue saveItem = new JumpstationGroupSelectorQueryParameterValue(true);
						saveItem.JumpstationGroupSelectorId = jumpstationGroupSelectorId;
		...
		private int? GetJumpstationGroupSelectorIdOfParent()
		{
