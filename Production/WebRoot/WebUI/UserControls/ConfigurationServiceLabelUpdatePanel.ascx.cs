...
{
	public partial class ConfigurationServiceLabelUpdatePanel : RecordDetailUserControl
	{
		#region Properties
		...
		private void PopulateListControls()
		{
			...
			Global.BindConfigurationServiceLabelTypeListControl(this.ddlLabelType, RowStatus.RowStatusId.Active);
			Global.BindConfigurationServiceItemListControl(this.ddlItem, RowStatus.RowStatusId.Active);
		}
		...
		private void ClearDataControls()
		{
			...
			this.ddlLabelType.ClearSelection();
			this.txtValueList.Text = string.Empty;
			this.txtSortOrder.Text = string.Empty;
			this.chkInputRequired.Checked = false;
		}
		...
		{
			ConfigurationServiceLabelQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		/// <returns></returns>
		public ConfigurationServiceLabelQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as ConfigurationServiceLabelQuerySpecification ?? new ConfigurationServiceLabelQuerySpecification(original);
		}
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					ConfigurationServiceLabel bindItem = ConfigurationServiceLabel.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
					...
					this.txtHelp.Text = bindItem.Help;
					this.ddlStatus.ForceSelectedValue((RowStatus.RowStatusId) bindItem.RowStatusId);
					this.ddlItem.ForceSelectedValue(bindItem.ConfigurationServiceItemId);
					this.ddlLabelType.ForceSelectedValue(bindItem.ConfigurationServiceLabelTypeId);
					this.chkInputRequired.Checked = (bool)bindItem.InputRequired;
					this.txtValueList.Text = bindItem.ValueList;
		...
		{
			ConfigurationServiceLabel saveItem;
			if (this.DataSourceId != null)
			{
				saveItem = ConfigurationServiceLabel.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new ConfigurationServiceLabel(true);
			}
			...
			saveItem.ConfigurationServiceItemId = Convert.ToInt32(this.ddlItem.SelectedValue);
			saveItem.ConfigurationServiceLabelTypeId = Convert.ToInt32(this.ddlLabelType.SelectedValue);
			saveItem.ValueList = this.txtValueList.Text.TrimToNull();
			saveItem.Help = this.txtHelp.Text.TrimToNull();
			saveItem.InputRequired = this.chkInputRequired.Checked;
            ...
            Global.RemoveConfigurationServiceLabelListControl(RowStatus.RowStatusId.Active);
		}
