...
{
	public partial class JumpstationMacroValueListPanel : BaseListViewUserControl
	{
		...
		{
            JumpstationMacroValueQuerySpecification query = this.ConvertToExpectedType(querySpecification);
            JumpstationMacroValueQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
            ...
            this.txtMatchName.DisableIf(!string.IsNullOrEmpty(immutableConditions.MatchName));
            this.txtMatchName.Text = query.MatchName;

            this.txtResultValue.DisableIf(!string.IsNullOrEmpty(immutableConditions.ResultValue));
            this.txtResultValue.Text = query.ResultValue;
		...
		{
			JumpstationMacroValueQuerySpecification query = new JumpstationMacroValueQuerySpecification();
			GridView gv = this.Grid;
            ...
            query.MatchName = this.txtMatchName.Text.TrimToNull();
            query.ResultValue = this.txtResultValue.Text.TrimToNull();
		...
		{
			this.EditJumpstationMacroValue(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapQueryParameterValue, VwMapQueryParameterValueCollection, QueryParameterValueQuerySpecification>(querySpecification, VwMapQueryParameterValueController.Fetch));
		}
		...
		/// </summary>
        public JumpstationMacroValueQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			this.EditJumpstationMacroValue(null);
		}
		...
		private void EditJumpstationMacroValue(int? recordId)
		{
			this.Response.Redirect(Global.GetJumpstationMacroValueUpdatePageUri(recordId, this.QuerySpecification));
		}
