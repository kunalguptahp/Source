...
{
	public partial class NoteTypeUpdatePanel : RecordDetailUserControl
	{
		#region Properties
		...
		{
			NoteTypeQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					NoteType bindItem = NoteType.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
		...
		protected override void SaveInput()
		{
			...
			{
				saveItem = NoteType.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new NoteType(true);
			}
		...
		private void UpdateNoteListImmutableConditions()
		{
            ...
            this.ucNoteList.ImmutableQueryConditions = new NoteQuerySpecification { NoteTypeId = (this.DataSourceId ?? -1) };
		}
