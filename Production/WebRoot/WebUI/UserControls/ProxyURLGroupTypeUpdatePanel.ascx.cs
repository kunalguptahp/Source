...
{
	public partial class ProxyURLGroupTypeUpdatePanel : RecordDetailUserControl
	{
		#region Properties
		...
		{
			ProxyURLGroupTypeQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					ProxyURLGroupType bindItem = ProxyURLGroupType.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
		...
		{
			ProxyURLGroupType saveItem;
			if (this.DataSourceId != null)
			{
				saveItem = ProxyURLGroupType.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new ProxyURLGroupType(true);
			}
            ...
            Global.RemoveProxyURLGroupTypeListControl(RowStatus.RowStatusId.Active);
		}
