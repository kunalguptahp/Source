...
{
	public partial class WorkflowApplicationTypeListPanel : BaseListViewUserControl
	{
		...
		{
			WorkflowApplicationTypeQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			WorkflowApplicationTypeQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			WorkflowApplicationTypeQuerySpecification query = new WorkflowApplicationTypeQuerySpecification();
			GridView gv = this.Grid;
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(query, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapWorkflowApplicationType, VwMapWorkflowApplicationTypeCollection, WorkflowApplicationTypeQuerySpecification>(query, VwMapWorkflowApplicationTypeController.Fetch));
		}
		...
		/// </summary>
		public WorkflowApplicationTypeQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		{
			this.Response.Redirect(Global.GetWorkflowApplicationTypeUpdatePageUri(applicationTypeId, this.QuerySpecification));
		}
		...
		/// <returns></returns>
		public WorkflowApplicationTypeQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as WorkflowApplicationTypeQuerySpecification ?? new WorkflowApplicationTypeQuerySpecification(original);
		}
