...
{
	public partial class ConfigurationServiceItemListPanel : BaseListViewUserControl
	{
		...
		{
			ConfigurationServiceItemQuerySpecification query = this.ConvertToExpectedType(querySpecification);
			ConfigurationServiceItemQuerySpecification immutableConditions = this.ConvertToExpectedType(this.ImmutableQueryConditions);
		...
		{
			ConfigurationServiceItemQuerySpecification query = new ConfigurationServiceItemQuerySpecification();
			GridView gv = this.Grid;
		...
		{
			this.EditGroupType(this.GetRowIdInt32(index));
		}
		...
		protected override void ExportData(IQuerySpecification querySpecification, string exportFormat, string filename)
		{
			...
			this.ExportData(querySpecification, exportFormat, filename, Global.YieldDataTablesForDataExport<VwMapConfigurationServiceItem, VwMapConfigurationServiceItemCollection, ConfigurationServiceItemQuerySpecification>(querySpecification, VwMapConfigurationServiceItemController.Fetch));
		}
		...
		/// </summary>
		public ConfigurationServiceItemQuerySpecification ConvertedQuerySpecification
		{
			get
			{
		...
		private void EditGroupType(int? groupTypeId)
		{
			this.Response.Redirect(Global.GetConfigurationServiceItemUpdatePageUri(groupTypeId, this.QuerySpecification));
		}
