...
{
	public partial class ConfigurationServiceItemUpdatePanel : RecordDetailUserControl
	{
		#region Properties
		...
		private void PopulateListControls()
		{
			...
			Global.BindConfigurationServiceGroupTypeListControl(this.chkConfigurationServiceGroupTypeList, RowStatus.RowStatusId.Active,tenantId,true);
		...
		private void ClearDataControls()
		{
			...
			this.chkConfigurationServiceGroupTypeList.ClearSelection();
			this.chkParent.Checked = false;
		...
		{
			ConfigurationServiceItemQuerySpecification defaultValuesSpecification = this.ConvertToExpectedType(this.DefaultValuesSpecification);
		...
		/// <returns></returns>
		public ConfigurationServiceItemQuerySpecification ConvertToExpectedType(IQuerySpecification original)
		{
			if (original == null)
			{
				return null;
			...
			return original as ConfigurationServiceItemQuerySpecification ?? new ConfigurationServiceItemQuerySpecification(original);
		}
		...
		protected override void BindItem()
		{
			...
			else
			{
				try
				{
					ConfigurationServiceItem bindItem = ConfigurationServiceItem.FetchByID(this.DataSourceId);
					if (bindItem == null)
					{
						this.Visible = false;
						return;
					...
					this.chkParent.Checked = bindItem.Parent;
					Global.SetConfigurationServiceGroupTypeControl(this.chkConfigurationServiceGroupTypeList, bindItem.GetConfigurationServiceGroupTypeCollection());
		...
		{
			ConfigurationServiceItem saveItem;
			if (this.DataSourceId != null)
			{
				saveItem = ConfigurationServiceItem.FetchByID(this.DataSourceId);
			}
			else
			{
				saveItem = new ConfigurationServiceItem(true);
			}

			saveItem.Name = this.txtName.Text.TrimToNull();
			saveItem.ElementsKey = this.txtElementsKey.Text.TrimToNull();
			saveItem.Parent = this.chkParent.Checked;
			saveItem.SortOrder = this.txtSortOrder.Text.TryParseInt32() ?? 0;
			...
			//create a list of the selected configuration service group types
			List<int> newConfigurationServiceGroupTypes = new List<int>();
			foreach (ListItem item in this.chkConfigurationServiceGroupTypeList.Items)
			{
				if (item.Selected)
				{
					int configurationServiceGroupTypeId = Convert.ToInt32(item.Value);
					newConfigurationServiceGroupTypes.Add(configurationServiceGroupTypeId);
				}
			}
			saveItem.SetConfigurationServiceGroupTypes(newConfigurationServiceGroupTypes);
            ...
            Global.RemoveConfigurationServiceItemListControl(RowStatus.RowStatusId.Active);
		}
		...
		{
			this.UpdateConfigurationServiceLabelListImmutableConditions();
		}

		private void UpdateConfigurationServiceLabelListImmutableConditions()
		{
			this.pnlConfigurationServiceLabelListPanel.Visible = this.DataSourceId != null;
			this.ucConfigurationServiceLabelList.ImmutableQueryConditions = new ConfigurationServiceLabelQuerySpecification { ConfigurationServiceItemId = (this.DataSourceId ?? -1) };
		}
